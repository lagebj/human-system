...
readonly installation_root="$data_home/human-system-agent-skills"
...
readonly instruction_file="$instruction_root/human-system-agent-skills.md"
...
link_repo_local_skill() {
  local repo_local_skills="$workspace/.agents/skills"

  [[ -d "$repo_local_skills" ]] || return 0
    ...
    for target in "$opencode_target" "$claude_target"; do
      if [[ -e "$target" || -L "$target" ]]; then
        if [[ -L "$target" ]]; then
          local resolved
          resolved="$(readlink -f "$target" 2>/dev/null || true)"
          if [[ "$resolved" == "$installation_root"/* || "$resolved" == "$workspace/.agents"/* ]]; then
            rm -f "$target"
          else
            warn "Keeping unmanaged skill '$skill_name' at $target."
            continue
