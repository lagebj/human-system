...
The threshold for intervention was clear: customer data at risk, regulatory violation, significant revenue loss. None of these thresholds were crossed. The slower response created cost. Whether the cost was proportionate remained unsettled.
...
What remained difficult after delegation: complex incidents that required system-level understanding. The on-call engineer could handle routine failures, but he could not handle cascading failures. These still required her. She documented this and set a date for reassessment. The date passed. She was still needed for cascading failures. She accepted this.

She thought about undocumented knowledge that stayed behind when someone left. The integration had worked, but some judgment had not transferred. It became a sunk cost. The organization paid it twice: once for the integration, once for the relearning. The staff engineer could have made the same mistake. She could have built an incident response system so complete that the team depended on her expertise to run it. Instead, she built something incomplete on purpose: the team could operate it without her, even if they operated it slower.

This incompleteness was intentional. Go-live was not completion. The staff engineer's go-live happened when the on-call engineer handled the cache invalidation without calling her. Completion would come later, when she left the team and the incident response continued without her. Or it would not come.
...
A change could succeed on paper and still create dependency. A change could fail to meet its goals and still leave capability behind. Capability mattered because the next result could not depend on the same expert arriving in time. Capability supported better outcomes. Capability did not excuse repeated, avoidable failure. Learning from failure did not erase the harm.

The work was building systems that did not require her constant presence, transferring capability that did not depend on her continued involvement, leaving behind people who could carry the work forward without her. This work was worth doing even when she would not be there to see it continue. The work accumulated. Departure would reveal what had actually been built.
